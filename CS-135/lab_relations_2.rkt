...
;; 5 PTS
(define (compose S R)
  (define (help1 S help2R)
    (cond
      [(null? help2R) '()]
      [else (append (list (append (list (car help2R)) (cdar S))) (help1 S (cdr help2R)))]
      )
    )

    (cond
      [(null? S) '()]
      [(not (null? (relate (caar S) (converse R)))) (make-set
                                                     (append (help1 S (relate (caar S) (converse R))) (compose (cdr S) R)))]
      [else (compose (cdr S) R)]
   )
  )
...
(define (tran R x)
  (cond ((or (null? R)(= 0 x)) '())
        (else (union (power x R) (tran R (- x 1))))))
...
(define (equivalence? n R)
  (cond ((and (symmetric? R) (transitive? R) (reflexive? n R)) #t)
        (else #f)))
...
(define (involutive? n R)
  (cond ((> n (length R)) #f)
        ((and (symmetric? R)(surjective? n R)(injective? (set-len n R))) #t)
        (else #f)))
(define (set-len n R)
  (cond ((= 0 n) '())
        (else (cons (car R) (set-len (- n 1) (cdr R))))))

(define (surjective? n R)
  (cond ((= 0 n) #t)
        ((> (length (relate n (converse R))) 0) (surjective? (- n 1) R))
        (else #f)))
