...
(define (mod-exp b e m)
  (cond [(= e 0) 1]
        [(even? e) (remainder (sqr (mod-exp b (/ e 2) m)) m)]
        [(odd? e) (remainder (* b (mod-exp b (- e 1) m)) m)]))
...
(define (encrypt plaintext key)
  (alter (string->list plaintext) key))

(define (alter inlist key)
  (if (null? inlist) '()
      (append (list (mod-exp (char->integer (car inlist)) (cadr key) (car key))) (alter (cdr inlist) key))))
...
(define (decrypt ciphertext key)
  (list->string (decode ciphertext key)))

(define (decode entered key)
  (if (null? entered) '()
      (append (list (integer->char (mod-exp (car entered) (cadr key) (car key)))) (decode (cdr entered) key))))
