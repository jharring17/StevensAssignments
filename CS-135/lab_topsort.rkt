...
(define (indegree E v)
  (if (null? E) 0
      (if (equal? (car (cdr (car E))) v)
          (+ 1 (indegree (cdr E) v)) (indegree (cdr E) v))))
...
;; 5 PTS
(define (find-top G)
  (topFind (get-edges G) (car G)))
   
(define (topFind E V)
  (if (null? V) (topsort-error)
      (if (equal? (indegree E (car V)) 0) (car V)
          (topFind E (cdr V)))))
...
;; 5 PTS
(define (topsort G)
  (if (null? (get-vertices G))
      (list) (append (list (find-top G)) (topsort (pop-vertex G (find-top G))))))
