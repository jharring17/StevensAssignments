...
(define (vertex g)
    (if (null? g)
        g
        (append (list (caar g) (cadar g)) (vertex (cdr g)))))
...
;; 3 PTS
(define (all-even? G)
  (compare G (get-vertices G)))

(define (compare g1 g2)
  (if (null? g2)
      #t
      (if (even? (degree g1 (car g2)))
          (compare g1 (cdr g2))
          #f)))
...
;; 4 PTS
(define (adjacent G v)
  (define (adjacent-help G v)
    (cond [(null? G) '()]
          [(= (caar G) v) (cons (cadar G) (adjacent-help (cdr G) v))]
          [(= (cadar G) v) (cons (caar G) (adjacent-help (cdr G) v))]
          [else (adjacent-help (cdr G) v)]))
  (usort (adjacent-help G v)))
...
;; 4 PTS
(define (connected? G)
  (define (conn-help G v)
    (cond [(null? v) #t]
          [(null? (remove (get-vertices G) (dfs G (car v)))) (conn-help G (cdr v))]
          [else #f]))
  (conn-help G (get-vertices G)))
